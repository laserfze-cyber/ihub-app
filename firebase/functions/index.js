...
function getDocIdBound(index, numBatches) {
  ...
  return "LOGIN/" + firstChar + secondChar;
}
...
exports.onUserDeleted = functions.auth.user().onDelete(async (user) => {
  ...
  await firestore.collection("LOGIN").doc(user.uid).delete();
});
const OneSignal = require("@onesignal/node-onesignal");

const kUserKey = "YZQ3CBXM2Q";
const kAPIKey = "pgwuu72usewxnrisajqg7ecrg";
...
exports.addUser = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const createdUser = await client.createUser(
      "82e2ce35-cf98-443f-8d5d-57a5872a5cbf",
      user,
    );
...
exports.deleteUser = functions.https.onCall(async (data, context) => {
  ...
  const url = `https://api.onesignal.com/apps/82e2ce35-cf98-443f-8d5d-57a5872a5cbf/users/by/external_id/${data.user_id}`;
